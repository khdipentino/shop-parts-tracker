...
export function AdjustStockForm({ partId }: { partId: string }) {
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} className="text-sm text-slate-500 dark:text-slate-400 underline">
  ...
  return (
    <form action={formAction} className="space-y-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
      ...
      <div className="flex gap-3">
        ...
        <button type="button" onClick={() => setOpen(false)} className="text-sm text-slate-500 dark:text-slate-400 underline">
          Cancel
        </button>
      </div>
    </form>
