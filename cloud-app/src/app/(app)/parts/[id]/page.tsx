...
export default async function PartDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <Link href="/parts" className="text-sm text-slate-500 underline">
        ← All parts
      </Link>
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-4">
          <span
            className={`text-2xl font-mono ${low ? "text-amber-600" : "text-ink"}`}
          >
            {part.quantity_on_hand}
          </span>
          <Link href={`/parts/${part.id}/label`} className="text-sm text-slate-500 underline">
            Print label
          </Link>
        </div>
      </div>
      ...
      <div>
        <h2 className="font-medium text-ink mb-3">History</h2>
            ...
            <tbody className="divide-y divide-slate-100">
              {(history ?? []).map((h) => (
                <tr key={h.id}>
                  ...
                  <td className="px-4 py-2 text-slate-500">{h.notes ?? ""}</td>
                </tr>
              ))}
