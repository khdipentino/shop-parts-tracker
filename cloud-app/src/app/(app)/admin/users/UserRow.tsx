      ...
      <tr className={!active ? "opacity-50" : ""}>
          ...
          {editingCode ? (
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-28 rounded-md border border-slate-300 bg-white px-2 py-1 text-sm font-mono"
              />
              <button onClick={saveCode} className="text-xs text-ink underline">
                Save
              </button>
              <button onClick={() => setEditingCode(false)} className="text-xs text-slate-500 underline">
                Cancel
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <span className="font-mono text-slate-700">{staffCode ?? "—"}</span>
              <button onClick={() => setEditingCode(true)} className="text-xs text-slate-500 underline">
                Edit
              </button>
            </div>
          )}
        ...
        <td className="px-4 py-2 align-top">
          <select
            ...
            className="rounded-md border border-slate-300 bg-white text-ink px-2 py-1 text-sm disabled:opacity-50"
          >
            ...
            <div className="rounded-md border border-amber-300 bg-amber-50 p-4 space-y-3">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="bg-brand text-white rounded-md px-3 py-1.5 text-sm font-medium hover:bg-brand-dark"
                >
                  Print badge
                </button>
                <button onClick={() => setGeneratedCode(null)} className="text-sm text-slate-500 underline">
                  Dismiss
                </button>
              </div>
            </div>
