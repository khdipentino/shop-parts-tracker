      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <section className="rounded-xl border border-slate-200 bg-white overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
            ...
            <Link href="/parts" className="text-xs text-slate-500 underline">
              All parts
            </Link>
          </div>
              ...
              {lowStockRows.map((p) => (
                <li key={p.id} className="px-4 py-2.5 flex items-center justify-between text-sm">
                  <Link href={`/parts/${p.id}`} className="min-w-0">
                    <p className="text-ink truncate">{p.description}</p>
                    <p className="text-slate-500 text-xs">{p.part_number}</p>
                  </Link>
                  <span className="shrink-0 rounded-full bg-amber-50 text-amber-700 px-2 py-0.5 text-xs font-medium">
        ...
        <section className="rounded-xl border border-slate-200 bg-white overflow-hidden">
          ...
          {!recent || recent.length === 0 ? (
            <p className="text-sm text-slate-500 px-4 py-6">Nothing yet.</p>
          ) : (
            <ul className="divide-y divide-slate-100">
              {recent.map((t) => (
                <li key={t.id} className="px-4 py-2.5 text-sm flex items-center justify-between gap-3">
                  ...
                  <span
                    className={`shrink-0 text-sm font-mono ${
                      t.quantity > 0 ? "text-emerald-600" : "text-slate-500"
                    }`}
                  >
